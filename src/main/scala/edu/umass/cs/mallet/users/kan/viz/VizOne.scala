...
object TwentyNewsgroupsOne  {
 
 val NUM_GROUPS:Int = 1 //20
 ...
 object Document {
   def readDocTopics(id: Int, label: String, pieces: Array[String]):Document = {
     ...
     var topics = new Array[Double](NUM_TOPICS)
 ...
 class Group(lbl: String) {
   ...
   var topicSums     = new Array[Double](NUM_TOPICS)
...
object VizOne {
 ...
 val file_doc_topics = "lda-doc-topics.txt" //"1000-dmr-iterations-10-topics/dmr-doc-topics.txt"
 //val file_state    = "/Users/jkan/umass/20news-bydate/state"
 val groups          = TwentyNewsgroupsOne.readDocTopics(file_doc_topics)
 ...
 abstract class Viewer extends PApplet {
   ...
   override def setup() = {
     //size(TwentyNewsgroupsOne.NUM_GROUPS*colwidth+borderwidth, 1024)
   ...
   override def draw = {
     ...
     def drawBg = {
       ...
       //val selection = mouseX * TwentyNewsgroupsOne.NUM_GROUPS / width
 ...
 object ViewByNewsgroup extends Viewer {
   ...
   class GroupView(group:TwentyNewsgroupsOne.Group) extends ThingView {
     ...
     //      text(group.label + " " + significant.mkString(" "), xbase, y, colwidth, ylineheight)
       ...
       def drawDocTopics(doc:TwentyNewsgroupsOne.Document, y:Int) = {
         for (i <- 0 until TwentyNewsgroupsOne.NUM_TOPICS) {
 ...
 //          line(x+thingwidth, y, x, y)
   ...
   def initViews: Array[ThingView] = groups.map[ThingView] ((group:TwentyNewsgroupsOne.Group) => new GroupView(group))
   def initNumThings = TwentyNewsgroupsOne.NUM_TOPICS
   def initNumBlocks = TwentyNewsgroupsOne.NUM_GROUPS
 }
 ...
 object ViewByTopic extends Viewer {
   ...
   class TopicView(topic:Int) extends ThingView {
     ...
     val topicGroups = groups.map[Double] {(g:TwentyNewsgroupsOne.Group) => g.topics(topic)}
     ...
     def draw: Unit = {
       def drawGroupHist = {
           ...
           for (g <- 0 until TwentyNewsgroupsOne.NUM_GROUPS) {
     ...
     //      text(group.label + " " + significant.mkString(" "), xbase, y, colwidth, ylineheight)
       ...
       def drawDocGroups(doc:TwentyNewsgroupsOne.Document, y:Int) = {
   ...
   def initViews: Array[ThingView] = {
     0.until(TwentyNewsgroupsOne.NUM_TOPICS).toArray.map[ThingView] ((topic:Int) => new TopicView(topic))
   }
   def initNumThings = TwentyNewsgroupsOne.NUM_GROUPS
   def initNumBlocks = TwentyNewsgroupsOne.NUM_TOPICS
 }
 ...
 def main(args : Array[String]) : Unit = {
   ...
   //val viz = new Viz("doc-topics.txt")
